...
1. **约多少钱**：把 token 折成美元，给"我的 agent 干了多少钱的活"的体感（契合"真实劳动→成长"的立意）。
...
## 每日预算提醒
- 用户在用量面板填 **每日预算 $**（`settings.dailyBudget`）。为空 = 不提醒。
...
## 数据（存档新增，向后兼容）
- `state.settings = { usdPerMillion: 1.3, dailyBudget: null }`（`defaultState` 补默认，旧存档合并后自动带上）。
...
- `formatMoney(n)` → `$12`、`$1,234`、`$3.4`（<10 保留一位小数）
