...
describe('stageFor（按累计消耗）', () => {
  ...
  it('阈值分界正确', () => {
    expect(stageFor(1 * B).key).toBe('sprout');
    expect(stageFor(3 * B).key).toBe('seedling');
    expect(stageFor(12 * B).key).toBe('bud');
    expect(stageFor(35 * B).key).toBe('fairy');
  });
  it('差一点不进化', () => {
    expect(stageFor(1 * B - 1).key).toBe('seed');
  });
  it('超过顶格仍是精灵', () => {
...
describe('progressFor', () => {
  it('顶格进度锁定 100%', () => {
    const p = progressFor(50 * B);
    expect(p.next).toBeNull();
    expect(p.fraction).toBe(1);
  });
  it('阶段中段进度约 0.5', () => {
    const mid = (3 * B + 12 * B) / 2;
...
describe('effectiveStage（累计消耗 + 空窗回落）', () => {
  it('活跃时等于累计消耗对应的形态', () => {
    const r = effectiveStage(5 * B, 0);
    expect(r.stage.key).toBe('seedling');
    expect(r.decayed).toBe(false);
  });
  it('超 24 小时没消耗退回一级', () => {
    const r = effectiveStage(5 * B, DECAY_MS + 1000);
  ...
  it('再蔫也不会低于种子', () => {
    const r = effectiveStage(2 * B, 10 * DECAY_MS);
